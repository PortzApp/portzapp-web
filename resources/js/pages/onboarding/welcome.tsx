...
export default function Welcome({ user }: Props) {
    const handleActionChoice = (action: 'create' | 'join') => {
        if (action === 'join') {
            toast.info('Join organization feature is coming soon!');
            return;
        }
    ...
    return (
        <OnboardingLayout user={user} title="Welcome - Choose Action">
            <div className="mx-auto mb-40 flex w-full max-w-5xl flex-1 flex-col justify-center">
                <div className="space-y-8">
                    ...
                    <div className="flex items-center justify-center gap-8">
                        <Card
                            className="h-fit w-full max-w-lg cursor-pointer border px-4 py-4 transition-all duration-200 hover:bg-muted hover:shadow-lg"
                        ...
                        <Card
                            className="relative h-fit w-full max-w-lg cursor-pointer border px-4 py-4 opacity-50 transition-all duration-200 hover:opacity-60"
                            onClick={() => handleActionChoice('join')}
                        >
                            <Badge className="absolute -top-2 -right-2 bg-amber-500 text-amber-950 hover:bg-amber-500">Coming Soon</Badge>
                            <CardHeader className="p-4">
                                <div className="flex flex-col items-start gap-6">
                                    <div className="flex rounded-xl bg-green-100 p-4 opacity-75">
                                        <Users className="size-8 text-green-600" />
                                    </div>
                                    <div className="flex-1">
                                        <CardTitle className="text-xl text-muted-foreground">Join Organization</CardTitle>
                                        <CardDescription className="mt-2 text-base text-balance text-muted-foreground">
