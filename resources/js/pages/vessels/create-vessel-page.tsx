...
export default function CreateVesselPage() {
    ...
    const [actionType, setActionType] = useState<'view' | 'another'>('view');

    const { data, setData, post, processing, errors, reset } = useForm<VesselForm>({
        name: '',
        imo_number: '',
        vessel_type: 'tanker_ship',
        status: 'active',
        grt: null,
        nrt: null,
        dwt: null,
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(`${route('vessels.store')}?action=${actionType}`, {
            onSuccess: () => {
                if (actionType === 'another') {
                    // Reset form for "Create & Add Another"
                    reset();
                }
                // For "view" action, the controller will redirect to the show page
                // For default behavior (no action), controller redirects to index
            },
        });
    };
            ...
            <form onSubmit={submit} className="flex flex-col gap-8 p-8">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-base text-muted-foreground">
                        Fill out the basic information to create a new vessel. All additional details are optional and can be added later.
                    </p>
                </div>
                ...
                <div className="flex gap-4">
                    <Button type="submit" disabled={processing} className="w-fit" onClick={() => setActionType('view')}>
                        {processing && actionType === 'view' && <LoaderCircle className="mr-2 h-4 w-4 animate-spin" />}
                        Create & View
                    </Button>
                    <Button type="submit" variant="outline" disabled={processing} className="w-fit" onClick={() => setActionType('another')}>
                        {processing && actionType === 'another' && <LoaderCircle className="mr-2 h-4 w-4 animate-spin" />}
                        Create & Add Another
                    </Button>
                </div>
            </form>
