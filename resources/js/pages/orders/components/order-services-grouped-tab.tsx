...
import { Building2, Copy, Eye, MoreHorizontal, Package, Tag, Users, DollarSign } from 'lucide-react';
...
export default function OrderServicesGroupedTab({ orderGroups }: OrderServicesGroupedTabProps) {
    ...
    const totalPrice = orderGroups.reduce((total, group) => {
        ...
        return (
            total +
            groupServices.reduce((sum, ogs) => sum + parseFloat(ogs.price_snapshot?.toString() || '0'), 0)
        );
    }, 0);
        ...
        <div className="space-y-8">
            ...
            <div>
                ...
                {orderGroups.map((group) => {
                    const groupServices = getGroupServices(group);
                    const groupTotalPrice = groupServices.reduce(
                        (sum, ogs) => sum + parseFloat(ogs.price_snapshot?.toString() || '0'),
                        0,
                    );
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    {groupServices.map((orderGroupService) => {
                                        ...
                                        const serviceTitle = 'sub_category' in service 
                                            ? service.sub_category?.name || 'Service' 
                                            : 'Service';
                                        
                                        const categoryName = 'sub_category' in service 
                                            ...
                                            <div key={orderGroupService.id} className="flex items-start justify-between rounded-lg border p-4">
                                                <div className="flex-1">
                                                    <div className="mb-2 flex items-center gap-2">
                                                        ...
                                                        {categoryName && (
                                                            <ServiceCategoryBadge categoryName={categoryName} />
                                                        )}
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="text-right">
                                                        <div className="text-lg font-semibold tabular-nums">
                                                            ${parseFloat(orderGroupService.price_snapshot?.toString() || '0').toLocaleString('en-US', { 
                                                    ...
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            <DropdownMenuItem onClick={() => router.visit(route('services.show', orderGroupService.service_id))}>
